...
<body class="bg-slate-100 font-sans">
    ...
    <div class="max-w-5xl mx-auto px-4 pt-28 pb-10">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-100">
                <h1 class="text-2xl font-bold text-slate-800">Riwayat Transaksi Saya</h1>
                <p class="text-sm text-slate-500 mt-1">Status pembayaran membership kamu.</p>
            </div>

            <div class="p-6 space-y-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <a href="{{ route('home') }}"
                        class="px-4 py-2 rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50">Keluar</a>

                    <form method="POST" action="{{ route('user.payments.clear') }}"
                        onsubmit="return confirm('Hapus semua riwayat transaksi kamu?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="px-4 py-2 rounded-lg bg-red-50 text-red-600 border border-red-100 hover:bg-red-100">
                            Hapus Riwayat
                        </button>
                    </form>

                    @if (!empty($activeMembershipEndsAt))
                        <div class="rounded-lg bg-slate-900 text-white px-4 py-2 text-sm">
                            Tenggat membership: <span id="membershipCountdown"
                                data-end="{{ $activeMembershipEndsAt }}T23:59:59">Menghitung...</span>
                        </div>
                    @endif
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($payments as $payment)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 capitalize">{{ $payment->membership_type ?? '-' }}</td>
                                    <td class="px-4 py-3">
                                        @if ($payment->status === 'lunas')
                                            <span
                                                class="px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs">Dikonfirmasi</span>
                                        @elseif ($payment->status === 'gagal')
                                            <span
                                                class="px-2 py-1 rounded-full bg-red-100 text-red-700 text-xs">Ditolak</span>
                                        @else
                                            <span
                                                class="px-2 py-1 rounded-full bg-amber-100 text-amber-700 text-xs">Pending</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-slate-600">
                                        {{ $payment->rejection_reason ?? ($payment->status === 'lunas' ? 'Selamat anda telah menjadi bagian dari kami!' : '-') }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-slate-500">Belum ada
                                        transaksi.
                                    </td>
                                </tr>
                            @endforelse
    ...
    <script>
        const countdownNode = document.getElementById('membershipCountdown');
        if (countdownNode) {
            const endDate = new Date(countdownNode.dataset.end).getTime();
            ...
            const renderCountdown = () => {
                ...
                if (distance <= 0) {
                    countdownNode.textContent = 'Waktu habis. Akun kembali menjadi user biasa.';
                    return;
                }
